...
Server.prototype.update = function (options) {
  ...
  this.battleeyeConfigurationOptions = options.battleeyeConfigurationOptions
...
Server.prototype.getBattleeyeConfigurationOptions = function () {
  var battleeyeConfigurationOptions = ''

  if (config.battleeyeConfigurationOptions) {
    battleeyeConfigurationOptions += config.battleeyeConfigurationOptions
  }

  if (this.battleeyeConfigurationOptions) {
    if (battleeyeConfigurationOptions) {
      battleeyeConfigurationOptions += '\n'
    }

    battleeyeConfigurationOptions += this.battleeyeConfigurationOptions
  }

  return battleeyeConfigurationOptions
}
...
Server.prototype.start = function () {
  ...
  if (config.virtualServer && config.virtualServer.enabled) {
    virtualServer.create(config, self.mods, self.id)
      .then((serverFolder) => {
...
Server.prototype.realStart = function (path) {
  ...
  server.writeBattleEyeConfig(this.getBattleeyeConfigurationOptions())
