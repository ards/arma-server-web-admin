...
function copyBattleEyeFolder (config, serverFolder) {

  const battleyeFolder = path.join(serverFolder, 'battleye')
  const battleyeServerFolder = path.join(config.path, 'battleye')

  return fs.promises.mkdir(battleyeFolder, { recursive: true })
    .then(() => {
  return fs.promises.readdir(battleyeServerFolder)
    .then((files) => {
      return Promise.all(files.map((file) => {
        return fs.promises.copyFile(path.join(battleyeServerFolder, file), path.join(battleyeFolder, file))
      }))
    })
  })
}
    ...
    return fs.promises.access(path.join(config.path, symlink))
      .then(() => {
        return fs.promises.symlink(path.join(config.path, symlink), path.join(serverFolder, symlink), 'junction')
          .catch((err) => {
            console.error('Could not create symlink for', symlink, 'due to', err)
          })
      })
      .catch(() => {})
  }))
...
module.exports.create = function (config, mods, serverId) {
  
  const serverFolder = path.join(os.tmpdir(), `arma-server-${serverId}`)

  if (fs.existsSync(serverFolder)) {
    console.log('Removing folder:', serverFolder)
    fs.rmdirSync(serverFolder, { recursive: true })
  }

  return fs.promises.mkdir(serverFolder)
    .then(() => {
      ...
      return Promise.all([
        ...
        copyBattleEyeFolder(config, serverFolder)
      ]).then(() => {
        return serverFolder
      })
    })
